...
def run_from_config(cfg: AppConfig) -> None:
    ...
    s3 = S3Client(region_name=cfg.aws_region, profile_name=cfg.aws_profile)
    ...
    indexer = HashIndexer(s3=s3, generator=gen)
    ...
    if cfg.output_target == "local_csv":
        ...
        print(f"Delivered index to Fabric landing at {cfg.output_path}")
        ...
        print(f"Upserted {len(df_all)} rows to Redshift {schema or 'public'}.{table}")

    # Update state
    for o in todo:
        state.mark_processed(o.key, o.etag)
    state.save(cfg.state_path)
    print(f"Updated state at {cfg.state_path}")
